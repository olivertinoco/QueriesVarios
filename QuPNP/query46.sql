...
insert into dbo.PROG_TARJETA_MULTIFLOTA(
Id_Vehiculo, Placa_Interna, Placa_Rodaje, Nro_Tarjeta, Fec_Activacion, activo, usuarioI, fechaI)
select s.Id_Vehiculo, s.Placa_Interna, s.Placa_Rodaje, s.Nro_Tarjeta, getdate(), 1, 'MASIVO', getdate()
from #tmp001_valores t, #tmp001_param s
where t.accion = 'UPDATE' and t.id_vehiculo = s.id_vehiculo

;with tmp001_sep(t,r,i)as(
    select*from(values('|','~','^'))t(sepCampo,sepReg,sepLst)
)
select isnull(stuff((select r, Placa_Interna, t, Placa_Rodaje, t, Nro_Tarjeta
