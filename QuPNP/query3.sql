...
cross apply(
    ...
    from(
        ...
        cross apply(
        select coalesce(xcol9, xcol8, xcol7, xcol6, xcol5, xcol4, xcol3, xcol2, xcol1) UltimaUnidad, CodUni
        from dbo.UNIDAD)tt where tt.CodUni = t.Id_UnidadDestino)t
        )t where t.item = 1 order by t.id offset 0 rows)t
    )t
    cross apply(select Id_TipoEstadoOpeVehiculo, id
    ...
    where t.Id_TipoEstadoOpeVehiculo = tt.Id_TipoEstado)t)t where t.item = 1 order by t.id offset 0 rows
    )tt where tt.id = t.id
)t
